...
public partial class UC_AdminUserManagement_uAdminUserNew : BaseUserControl
{
    ...
    protected void FillData() {
        DataTable dt = AdminPermissions.GetAdmin(this.AdminID);
        if (dt.Rows.Count > 0) {
            hdAdminID.Value = this.AdminID.ToString();
            txtUserName.Text = dt.Rows[0]["UserName"].ToString();
            txtPassword.Text = Encryption.Decrypt(dt.Rows[0]["Password"].ToString());
            ...
            this.dtUserPermission = AdminPermissions.GetAdminUserPermission(this.AdminID);
    ...
    {
        DataBindHelper.BindRepeater(ref rptPermissionList, AdminPermissions.AdminPermissionsSel());        
    }
                ...
                foreach (Control c in e.Controls)
                {
                    ...
                    {                        
                        ...
                        if (!IsSaveClick)
                        {
                            ...
                            if (this.AdminID > 0)
                                ch.Checked = DataTableHelper.ExistValue(this.dtUserPermission, "AdminPermissionID", PermissionID.ToString());
                        }
    ...
    {
        uError2.Visible = ((int.Parse(hdAdminID.Value) == 0) && (AdminPermissions.HasAdmins(txtUserName.Text)));
        if (!uError2.Visible)
        {
            ArrayList arr = new ArrayList(hdPermissionChecked.Value.Split(new string[] { ", " }, StringSplitOptions.RemoveEmptyEntries));
            bool IsSave =(AdminPermissions.AdminUserNew(int.Parse(hdAdminID.Value), txtUserName.Text, txtPassword.Text, txtFirstname.Text, txtSurName.Text, txtDescription.Text, this.smAdminID, rdActive.Checked, arr));
            if (IsSave)
            {
