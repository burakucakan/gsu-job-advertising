...
public partial class UC_Management_uNavigate : BaseUserControl
{
    ...
    {
        if (!this.HasUserPermission(Util.ReturnPageName()))
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            DataTable dtNavigate = DataTableHelper.Filter(this.smPermissions, "IsNavigate", "True");
            DataBindHelper.BindRepeater(ref rptMenu, dtNavigate);
        }
    }
